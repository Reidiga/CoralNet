...
```
# Create a long format and convert to long format
```{r}
...
p.cover_long <- p.cover_long %>%
  mutate(month.number = substr(Name, 3,3),
         Month = ifelse(month.number == "6", "July",
                        ifelse(month.number == "7","July","August")))
```
...
(algae_p.cover <- ggplot(data = p.cover_long,
  ...
  facet_grid(Month ~ Treatment_area) +
